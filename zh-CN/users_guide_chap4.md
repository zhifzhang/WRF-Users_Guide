...
Real数据案例是将WPS提供的输入数据用real.exe程序运行。来自WPS的数据由之前运行的外部分析或预测模型生成。原始数据很可能是[GriB](http://www.wmo.int/pages/prog/www/WDM/Guides/Guide-binary-2.html )格式的，而且很可能是通过首先从国家气象局的匿名ftp站点中提取原始GriB数据而传入WPS的。

例如，假设要用WRF按照以下要求进行单层嵌套模拟：
